...
```elixir
boardroom = Timetable.resource("Boardroom",
...
```elixir
boardroom = Timetable.open(boardroom, ~o"R/2026-01-05T08/PT10H/P1W", on: Tempo.workdays())
```
...
Timetable.travel_time(boardroom, annexe)      # both within level_2
# => ~o"PT3M"

Timetable.travel_time(boardroom, main_stage)  # different venues
# => ~o"PT25M"
```
...
```elixir
review =
  Timetable.session("Quarterly review", lasting: ~o"PT1H")
  |> Timetable.needs(:room, seats: at_least(8), video_conferencing: true)
  |> Timetable.needs(:attendees, all_of([alice, bob, carol]))
  |> Timetable.between(~o"2026-06-15/2026-06-20")
  |> Timetable.prefers(within: sydney)
```
...
```elixir
elixir_track =
  Timetable.track("Elixir", of: [keynote, otp_internals, ecto_at_scale])
  |> Timetable.no_overlap()
  |> Timetable.reachable(within: ~o"PT10M")
```
...
```elixir
programme =
  Timetable.programme("ElixirConf AU 2026")
  |> Timetable.across(~o"2026-09-15/2026-09-18")
  |> Timetable.in_places([sydney, darling])
  |> Timetable.add_track(elixir_track)
  |> Timetable.add_track(phoenix_track)

{:ok, arrangement} = Timetable.arrange(programme, against: ledger)
```
...
```elixir
Timetable.explain(review, meeting_room_2)
...
```elixir
{:ok, arrangements} = Timetable.plan(review, against: ledger)

Enum.map(arrangements, &Timetable.explain/1)
...
```elixir
{:error, %Timetable.Infeasible{} = reason} = Timetable.plan(unstaffable, against: ledger)
...
```elixir
ledger = Timetable.Ledger.new()
{:ok, ledger} = Timetable.allocate(ledger, arrangement)

Timetable.free(ledger, boardroom, during: ~o"2026-06-16")
...
```elixir
{:ok, changes} = Timetable.rearrange(ledger, "Quarterly review", new_arrangement)
