# `ash_scheduling_agenda` — a discussion document
...
**Note on circulation:** this document describes internals of `team-alembic/ash_scheduling`, which is a private repository. It should stay internal until Alembic are happy for it to travel — in particular it should not ship in a public release of `agenda`.

**Basis:** `ash_scheduling` at `main` as of 2026-08-03 (last commit 2026-07-03), read directly: `lib/ash_scheduling/schedule_engine.ex`, the DSL docs, ADR-002, and `internal_docs/architecture/2026-02-09-implementation-requirements.md`. `agenda` at `336ce4e`.

## Why an adapter at all

The two libraries solve adjacent halves of the same problem and barely overlap.

`ash_scheduling` owns everything to do with **being an Ash application**: resources, relationships, queries, transactions, policies, status lifecycle, persistence. Its `ScheduleEngine` resolves availability over that data — expanding RRULEs per schedule or availability timezone, subtracting block-outs and claims, intersecting across schedules for the all-free case. None of that can live in a library that does not know Ash, and `agenda` does not.

`agenda` owns everything to do with **what resources are and which one to choose**: typed attributes, requirement matching with explanations, a place tree that derives travel time, capacity depth, turnaround buffers, and a backtracking arranger for laying out many sessions at once. It is a pure value library — no processes, no database, no clock — sitting on Tempo's calendar-, timezone-, and DST-correct interval algebra.

The seam is narrow and the vocabularies already agree, which is the strongest evidence the fit is real:

| AshScheduling | Agenda |
| --- | --- |
...
**`agenda`.** All four of those are implemented and tested, along with attribute matching, the place tree, series expansion, and whole-programme arrangement. It has no notion of persistence, concurrency control, or a clock.
...
The adapter implements `AshScheduling.Strategy` (and `Matcher`, `Selector`, `CostFn` if and when those modules land), backed by Agenda's matching, ranking, and place-tree travel. `ScheduleEngine` is untouched.
...
As A, and additionally the adapter post-processes `ScheduleEngine.resolve/3` output through Agenda to add capacity depth, buffers, and `min_duration` filtering.

* **For:** the four unbuilt engine capabilities arrive without `ash_scheduling` implementing them. The engine keeps ownership of Ash data loading, RRULE expansion, and status filtering — the parts Agenda genuinely cannot do.
...
Agenda's availability computation replaces the resolution core of `ScheduleEngine`, with `ash_scheduling` keeping data loading, the DSL, and the write path.
...
### 1. Projecting Ash resources into Agenda resources
...
The adapter should **not** add an attributes column. ADR-002's "your domain stays yours" is right, so the consumer implements a small behaviour projecting *their own* resource into a Agenda resource:
...
* `Strategy.select/3` — filter pool members through Agenda requirement matching, rank by preference, return the chosen member(s). The multi-role case (one doctor *and* one room) is what `plan/3` already does.
...
4. **Capacity representation.** Your `Claim` carries a `quantity`; Agenda counts overlapping claims and treats a quantity of *n* as *n* claims. Is that encoding acceptable at the boundary, or should Agenda take weights directly? The latter is a small change and we would rather match your model.
...
6. **Holds, and the clock.** This is the one capability Agenda deliberately does not have. A `:held` claim consumes availability until it expires, so an engine has to know about it, and that needs an injectable clock. We did not build one because **how a hold is represented depends on how claims are persisted, which is your decision.** You already have `AshScheduling.Clock`. Should Agenda grow a compatible clock abstraction, or should holds stay entirely on your side of the seam and simply arrive as busy time?
