defmodule Agenda.Preference do
  @moduledoc """
  ...
  @builtin [:room_changes, :room_spread, :resource_wishes]
  ...
  * `preference` is a built-in name — `:room_changes`, `:room_spread`
    or `:resource_wishes` — or a `{name, function}` pair for one of
    your own.
  ...
  * one sentence per preference, in the order they were declared.
  ...
  defp counter(:resource_wishes), do: &resource_wishes/2

  # What each resource would rather. `avoids` and `prefers` on a
  # `Agenda.Resource` are wishes, not rules — unlike `open/2`, which
  # says a resource *cannot* be used, these say it would rather not be,
  # and only bite when a programme asks for them.
  #
  # This is the per-resource half of preferences. The others score a
  # layout as a whole; this one asks each resource in it whether it is
  # content, which is how "Ann would rather not work Tuesdays" is said
  # without making Tuesday unavailable to everyone.
  defp resource_wishes(arrangements, _context) do
    Enum.reduce(arrangements, 0, fn arrangement, total ->
      total +
        Enum.count(
          Arrangement.resources(arrangement),
          &unwilling?(&1, arrangement.interval)
        )
    end)
  end

  defp unwilling?(%Resource{avoids: nil, prefers: nil}, _interval), do: false

  defp unwilling?(%Resource{} = resource, interval) do
    overlaps?(resource.avoids, interval) or
      (resource.prefers != nil and not overlaps?(resource.prefers, interval))
  end

  # A wish may be written as a recurrence — "any Tuesday" — so it is
  # materialised against the placement being judged rather than needing
  # a bound of its own. An unreadable wish scores nothing rather than
  # crashing a scoring pass: a preference must never be able to fail an
  # arrangement that is otherwise valid.
  defp overlaps?(nil, _interval), do: false

  defp overlaps?(pattern, interval) do
    with {:ok, value} <- Availability.normalise(pattern),
         {:ok, materialised} <- Tempo.to_interval(value, bound: interval),
         {:ok, shared} <- Tempo.intersection(materialised, interval) do
      not empty?(shared)
    else
      _unreadable -> false
    end
  ...
  defp empty?(%IntervalSet{} = set), do: IntervalSet.count(set) == 0
