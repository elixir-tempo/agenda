defmodule Agenda.Layout do
  @moduledoc """
  A programme laid out as far as it would go — what was placed, and
  what was left out.

  `Agenda.Arranger.arrange/3` fails the whole programme by default:
  ...
  Each entry in `unplaced` is a `t:Agenda.Infeasible.t/0`, so a
  ...
  * `false` — the best found before the search hit its `:nodes` cap.
  ...
  `Agenda.Preference`s, where `0` is ideal. `score_proven?` is its
  ...
  * `score_proven?` — did the scoring pass finish, or stop at its
  ...
  `Agenda.explain_score/3` breaks the number down per preference,
  ...
  * `placed` is the list of `t:Agenda.Arrangement.t/0` that were
    successfully placed.

  * `unplaced` is a non-empty list of `t:Agenda.Infeasible.t/0`, one
