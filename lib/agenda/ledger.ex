defmodule Agenda.Ledger do
  ...
  * **Releasing is dropping a key.** Cancelling a session frees every
    ...
    free time is derived by `Agenda.Availability.free/2`, never
    stored.
  ...
  * the allocations, ordered by session then role then resource so the
  ...
  `Agenda.arrange/3` takes `t:Agenda.Arrangement.t/0` values as
  ...
  * `pool` is the list of `t:Agenda.Resource.t/0` the allocations
    were made from.
  ...
  * `{:error, t:Agenda.Infeasible.t/0}` naming the resources the
    pool does not have.
  ...
  * `:except` is a session name, or a list of them, whose allocations
    ...
    pinning sessions in `Agenda.arrange/3`, which claims what its
  ...
  * `arrangement` is the `t:Agenda.Arrangement.t/0` it should move
    to.
