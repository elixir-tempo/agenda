defmodule Timetable.Series do
  ...
  ### Example
      ...
      iex> standup = Timetable.session("Stand-up", lasting: "PT15M")
      iex> {:ok, [first | _]} = Timetable.every(standup, "R3/2027-03-02T09:00:00/P1W")
  ...
  * `session` is the `t:Timetable.Session.t/0` to repeat. Its own
  ...
  * `{:error, reason}` when the pattern cannot be read or is unbounded
  ...
  ### Examples
      ...
      iex> clinic = Timetable.session("Clinic", lasting: "PT4H")
      iex> {:ok, occurrences} = Timetable.Series.expand(clinic, "R2/2027-03-02/P1D")
      iex> length(occurrences)
      2

      iex> clinic = Timetable.session("Clinic", lasting: "PT4H")
      iex> Timetable.Series.expand(clinic, "not a recurrence")
