if Code.ensure_loaded?(CPSolver) do
  defmodule Timetable.Fixpoint do
    @moduledoc """
    Hand a programme to a real constraint solver, and take the answer
    back as arrangements.

    `Timetable.Arranger.arrange/3` is exact and explains itself, and it
    ...
    result back through `Timetable.Ledger.allocate/2`". This module is
    ...
    `t:Timetable.Arrangement.t/0` values that the ledger accepts
    ...
    of candidate placements from `Timetable.Planner.plan/3` — every
    ...
    Conflicts come from `Timetable.Arranger.conflict?/4` — the same
    ...
    **Partial layouts and preferences.** This answers the
    ...
    `Timetable.Preference` all stay with `arrange/3`.
    ...
    * `:busy` is a map of resource name to what already claims it, as
      `Timetable.Ledger.busy/2` returns.
    ...
    * `{:ok, arrangements}` — one per session, in programme order,
      ready for `Timetable.Ledger.allocate/2`; or

    * `{:error, t:Timetable.Infeasible.t/0}` when a session has no
    ...
    ### Examples
        ...
        iex> {:ok, arrangements} = Timetable.Fixpoint.solve(programme, [room])
        iex> length(arrangements)
        2

    """
    @spec solve(Programme.t(), [Resource.t()], keyword()) ::
            {:ok, [Timetable.Arrangement.t()]} | {:error, Infeasible.t()}
           ...
           Infeasible.new(
             ...
             Enum.map(
               ...
               &("#{&1.name} has concurrency #{&1.concurrency} — the fixpoint bridge " <>
                   "models exclusive resources only; use Timetable.arrange/3 instead")
             )
           )}
      end
    end
    ...
    # `Timetable.Arranger.conflict?/4`, so the solver is answering the
    ...
    defp no_layout(programme) do
      Infeasible.new(programme.name, [
        "the solver proved no arrangement places every session — " <>
          "Timetable.conflict/3 will name the sessions in tension"
      ])
    end
  end
end
