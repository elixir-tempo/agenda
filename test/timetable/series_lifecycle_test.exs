defmodule Timetable.SeriesLifecycleTest do
  ...
  setup do
    {:ok, room} =
      Timetable.resource("Huddle", seats: 6)
      |> Timetable.open("R28/2027-03-01T09:00:00/P1D")

    standup =
      Timetable.session("Stand-up", lasting: ~o"PT15M")
      |> Session.needs(:room, seats: 6)

    {:ok, occurrences} = Timetable.every(standup, "R4/2027-03-01T09:00:00/P1W")

    %{room: room, occurrences: occurrences}
  end

  defp book_all(occurrences, room) do
    Enum.reduce(occurrences, Timetable.ledger(), fn occurrence, ledger ->
      {:ok, [first | _]} = Timetable.plan(occurrence, [room], busy: Timetable.busy(ledger))
      {:ok, ledger} = Timetable.allocate(ledger, first)
      ledger
    end)
  end
  ...
  describe "expansion" do
    ...
    test "an unreadable pattern is an error, not a crash" do
      session = Timetable.session("X", lasting: ~o"PT1H")

      assert Timetable.every(session, "not a recurrence") == {:error, :unreadable_pattern}
    end
  end
  ...
  describe "release_series/3" do
    ...
    test "leaves other sessions alone", context do
      ...
      one_off =
        Timetable.session("Retro", lasting: ~o"PT1H", between: "2027-03-02/2027-03-03")
        |> Session.needs(:room, seats: 6)

      {:ok, [first | _]} = Timetable.plan(one_off, [context.room], busy: Timetable.busy(ledger))
      {:ok, ledger} = Timetable.allocate(ledger, first)
    ...
    test "releasing frees the exact slots the run had taken", context do
      ...
      {:ok, held} =
        Timetable.plan(second, [context.room], busy: Timetable.busy(ledger), limit: 500)
