defmodule Timetable.ConcurrencyTest do
  ...
  defp resource(name, concurrency) do
    {:ok, resource} =
      Timetable.resource(name, seats: 1, concurrency: concurrency)
      |> Timetable.open(@morning)
  ...
  describe "plan/3 honours concurrency" do
    defp hire(pool, busy) do
      session =
        Timetable.session("Locker hire", lasting: ~o"PT1H", between: ~o"2027-03-02/2027-03-03")
        |> Session.needs(:room, seats: at_least(1))

      {:ok, options} = Timetable.plan(session, pool, busy: busy)
      options
    end
  ...
  describe "arrange/3 honours concurrency" do
    defp booking(name) do
      Timetable.session(name, lasting: ~o"PT1H")
      |> Session.needs(:desk, seats: at_least(1))
    end

    defp programme_of(names) do
      Enum.reduce(names, Timetable.programme("P", across: @window), fn name, acc ->
    ...
    test "sessions may share a resource up to its concurrency" do
      ...
      {:ok, desk} =
        Timetable.resource("Desk", seats: 1, concurrency: 3)
        |> Timetable.open("2027-03-02T09:00:00/2027-03-02T10:00:00")
    ...
    test "a concurrency-1 resource still serialises" do
      {:ok, desk} =
        Timetable.resource("Desk", seats: 1)
        |> Timetable.open("2027-03-02T09:00:00/2027-03-02T11:00:00")
