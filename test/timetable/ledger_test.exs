defmodule Timetable.LedgerTest do
  ...
  doctest Timetable.Ledger
  doctest Timetable.Allocation

  setup do
    %{
      boardroom: Timetable.resource("Boardroom"),
      annexe: Timetable.resource("Annexe"),
      alice: Timetable.resource("Alice"),
      bob: Timetable.resource("Bob"),
  ...
  describe "the whole loop — plan, allocate, re-plan" do
    setup do
      ...
      {:ok, boardroom} = Timetable.open(boardroom, "2026-06-15T09:00:00/2026-06-15T12:00:00")
    ...
    test "an allocated slot is not offered again", context do
      {:ok, arrangements} = Timetable.plan(context.session, [context.room])
      assert length(arrangements) == 3

      {:ok, ledger} = Ledger.allocate(Ledger.new(), hd(arrangements))

      {:ok, remaining} =
        Timetable.plan(context.session, [context.room], busy: Ledger.busy(ledger))

      assert length(remaining) == 2
    end

    test "releasing puts the slot back", context do
      {:ok, [first | _]} = Timetable.plan(context.session, [context.room])
    ...
    test "re-planning a session sees past its own allocation", context do
      ...
      {:ok, options} =
        Timetable.plan(context.session, [context.room],
          busy: Ledger.busy(ledger, except: "Review")
        )
