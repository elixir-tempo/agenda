defmodule Timetable.VAvailabilityTest do
  ...
  defp room_open_from(components, name \\ "Room") do
    {:ok, hours} = Timetable.from_ical(calendar(components))
    {:ok, room} = Timetable.open(Timetable.resource(name, seats: 8), hours)
    room
  end
  ...
  describe "importing open hours" do
    test "a VAVAILABILITY becomes a resource's open hours" do
      ...
      assert {:ok, free} = Timetable.free(room, within: @june)
    ...
    test "the pattern is held unmaterialised until a window is known" do
      ...
      {:ok, whole_week} = Timetable.free(room, within: @june)
      {:ok, two_days} = Timetable.free(room, within: "2026-06-01/2026-06-03")
    ...
    test "PRIORITY is honoured through the Timetable layer" do
  ...
  describe "imported hours compose with the rest of the library" do
    test "planning against imported availability works" do
      ...
      session =
        Timetable.session("Review", lasting: "PT1H", between: @june)
        |> Session.needs(:room, seats: 8)

      assert {:ok, [best | _rest]} = Timetable.plan(session, [room])
    ...
    test "refinements apply to imported availability" do
      ...
      assert {:ok, mornings} =
               room
               |> Timetable.free(within: @june)
               |> Timetable.only_during("2026-06-01T09:00:00/2026-06-01T12:00:00")
  ...
  describe "bad input" do
    ...
    test "unreadable data is an error, not a crash" do
      assert {:ok, hours} = Timetable.from_ical("this is not a calendar")
      {:ok, room} = Timetable.open(Timetable.resource("Room"), hours)
    ...
    test "normalise/1 accepts an imported pattern unchanged" do
      {:ok, hours} = Timetable.from_ical(calendar(weekdays()))
