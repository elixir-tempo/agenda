defmodule Timetable.PreferenceTest do
  ...
  setup do
    ...
    open = fn resource ->
      {:ok, resource} = Timetable.open(resource, "2026-09-15T09:00:00/2026-09-15T13:00:00")
      resource
    end
  ...
  describe "declaring preferences" do
    test "a built-in is recognised" do
      assert {:ok, programme} = Timetable.prefer(Timetable.programme("Conf"), :room_changes)
      assert [%Timetable.Preference{name: :room_changes, weight: 1}] = programme.preferences
    end

    test "a weight is carried" do
      assert {:ok, programme} =
               Timetable.prefer(Timetable.programme("Conf"), :room_spread, weight: 7)
    ...
    test "an unknown name is an error, not silently ignored" do
      assert {:error, {:unknown_preference, :teleportation}} =
               Timetable.prefer(Timetable.programme("Conf"), :teleportation)
    end
    ...
    test "a custom preference is a name and a function" do
      ...
      assert {:ok, programme} =
               Timetable.prefer(Timetable.programme("Conf"), {:count_them, counter}, weight: 2)
  ...
  describe "room_changes" do
    test "a track that stays put beats one that moves", context do
      ...
      {:ok, programme} = Timetable.prefer(conf([track]), :room_changes, weight: 10)

      assert {:ok, arrangements} = Timetable.arrange(programme, [context.hall, context.studio])
    ...
    test "the score is zero when nothing moves", context do
      ...
      {:ok, arrangements} = Timetable.arrange(programme, [context.hall, context.studio])

      assert Timetable.score(arrangements, programme) == 0
    end

    test "untracked sessions are not counted — they share no audience", context do
      {:ok, programme} =
        Timetable.prefer(conf([talk("One"), talk("Two")]), :room_changes, weight: 10)
  ...
  describe "room_spread" do
    test "it prefers using both rooms over piling into one", context do
      ...
      {:ok, programme} =
        Timetable.prefer(conf([talk("One"), talk("Two")]), :room_spread, weight: 10)
    ...
    test "one room in use is not an imbalance", context do
      {:ok, programme} = Timetable.prefer(conf([talk("Only")]), :room_spread, weight: 10)

      {:ok, arrangements} = Timetable.arrange(programme, [context.hall])
  ...
  describe "the lexicographic guarantee" do
    test "a preference never costs a placement", context do
      ...
      {:ok, preferred} = Timetable.prefer(programme, :room_spread, weight: 100)

      assert {:ok, plain} = Timetable.arrange(programme, [context.hall])
      assert {:ok, scored} = Timetable.arrange(preferred, [context.hall])
    ...
    test "and never costs one in a partial layout either", context do
      ...
      assert {:partial, plain} =
               Timetable.arrange(programme, [context.hall], unplaced: :allow)

      assert {:partial, scored} =
               Timetable.arrange(preferred, [context.hall], unplaced: :allow)
  ...
  describe "score_proven?" do
    test "a completed search proves its score", context do
      ...
      {:ok, programme} = Timetable.prefer(conf(Enum.map(1..6, &talk("S#{&1}"))), :room_changes)
    ...
    test "hitting the node cap leaves the score unproven", context do
      {:ok, programme} = Timetable.prefer(conf(Enum.map(1..6, &talk("S#{&1}"))), :room_spread)

      case Timetable.arrange(programme, [context.hall], unplaced: :allow, nodes: 12) do
  ...
  describe "explaining a score" do
    test "each preference says what it contributed", context do
      ...
      assert [changes, spread] = Timetable.explain_score(arrangements, programme)
    ...
    test "a layout can be explained directly", context do
      ...
      {:partial, layout} = Timetable.arrange(programme, [context.hall], unplaced: :allow)

      assert [sentence] = Timetable.explain_score(layout, programme)
  ...
  describe "no preferences costs nothing" do
    test "behaviour is identical to before", context do
      ...
      assert {:ok, arrangements} = Timetable.arrange(programme, [context.hall])
