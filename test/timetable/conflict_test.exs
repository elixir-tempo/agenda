defmodule Timetable.ConflictTest do
  ...
  doctest Timetable.Conflict

  defp room(name, hours, attributes \\ []) do
    {:ok, room} =
      Timetable.resource(name, [{:seats, 100} | attributes])
  ...
  describe "conflict/3 for a programme" do
    ...
    test "pinned sessions are background, never named" do
      ...
      {:ok, arrangements} = Timetable.plan(pick(programme, "A"), pool)
  ...
  describe "conflict/3 for a session" do
    test "nothing to explain when the session can be held" do
      ...
      session =
        Timetable.session("Review", lasting: "PT1H", between: "2026-09-15/2026-09-16")
    ...
    test "an induced requirement is named alongside the demand it fights" do
      ...
      {:ok, alice} = Timetable.open(alice, "2026-09-15T09:00:00/2026-09-15T12:00:00")
    ...
    test "no demand is to blame when nothing is free" do
      ...
      session =
        Timetable.session("Review", lasting: "PT3H", between: "2026-09-15/2026-09-16")
        |> Session.needs(:room, seats: at_least(8), video_conferencing: true)
  ...
  describe "Timetable.conflict/3 dispatches on what it is given" do
    test "a programme" do
      programme = conf([talk("Keynote"), talk("Deep dive")])

      assert {:ok, conflict} = Timetable.conflict(programme, [room("Hall", 1)])
      assert Enum.sort(conflict) == ["Deep dive", "Keynote"]
    end
    ...
    test "a session" do
      ...
      assert Timetable.conflict(session, [room("Hall", 3)]) == {:ok, [needs: {:room, :seats}]}
    end
  end
end
