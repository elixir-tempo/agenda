defmodule Timetable.PlannerTest do
  ...
  doctest Timetable.Planner

  setup do
    sydney = Timetable.place("Sydney")
    level_2 = Timetable.place("Level 2", within: sydney)
    level_3 = Timetable.place("Level 3", within: sydney)
    darling = Timetable.place("Darling Harbour")

    open = fn resource ->
      {:ok, resource} = Timetable.open(resource, "2026-06-15T09:00:00/2026-06-15T17:00:00")
      resource
    end
    ...
    %{
      ...
      annexe: open.(Timetable.resource("Annexe", within: level_3, seats: 12)),
      attic: open.(Timetable.resource("Attic", within: darling, seats: 12)),
      small: open.(Timetable.resource("Meeting room 2", within: level_2, seats: 4)),
      alice: open.(Timetable.resource("Alice", requires: [step_free_access: true])),
      bob: open.(Timetable.resource("Bob"))
    }
  end

  defp review(context, extra \\ []) do
    "Review"
    |> Timetable.session(lasting: "PT1H", between: "2026-06-15/2026-06-16")
  ...
  describe "plan/3 — busy time is subtracted" do
    ...
    test "everyone busy is infeasible, with a reason", context do
      ...
      assert Timetable.explain(reason) =~ "Review cannot be held"
    end
  end
  ...
  describe "plan/3 — eligibility and induced requirements" do
    ...
    test "an attendee's own requirement rules out a room", context do
      ...
      assert Timetable.explain(reason) =~ "step_free_access"
    end
