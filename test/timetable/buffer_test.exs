defmodule Timetable.BufferTest do
  ...
  defp room(options) do
    {:ok, room} = Timetable.open(Timetable.resource("Room", options), @day)
    room
  end
  ...
  describe "plan/3 respects turnaround" do
    test "a slot butting up against a claim's buffer is not offered" do
      ...
      session =
        Timetable.session("Meeting", lasting: ~o"PT1H", between: ~o"2027-03-02/2027-03-03")
        |> Session.needs(:room, [])

      {:ok, options} =
        Timetable.plan(session, [cleaned],
