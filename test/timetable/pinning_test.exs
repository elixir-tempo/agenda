defmodule Timetable.PinningTest do
  ...
  defp placement(programme, pool, session, nth) do
    {:ok, arrangements} = Timetable.plan(pick(programme, session), pool, limit: 10)
  ...
  describe "pins are constrained like anything else" do
    ...
    test "a pin counts against a track's reachability", context do
      ...
      # fails.
      {:ok, annexe} =
        Timetable.resource("Far Room", within: Timetable.place("Annexe Building"), seats: 100)
    ...
    test "a pinned session still respects concurrency" do
      {:ok, bank} =
        Timetable.resource("Lockers", seats: 100, concurrency: 2)
        |> Timetable.open("2026-09-15T09:00:00/2026-09-15T10:00:00")
  ...
  describe "a pin that cannot be honoured is an error" do
    test "naming a session the programme does not have", context do
      ...
      assert Timetable.explain(reason) =~ "Ghost is pinned but is not in the programme"
    end
    ...
    test "pinning the same session twice", context do
      ...
      assert Timetable.explain(reason) =~ "pinned 2 times"
    end
    ...
    test "two pins that hold the same room at the same time", context do
      ...
      assert Timetable.explain(reason) =~ "clashes with another pin"
    end
  end
  ...
  describe "pinning what is already booked" do
    test "a ledger round-trips into pins", context do
      ...
      {:ok, ledger} = Timetable.allocate(Timetable.ledger(), first)
    ...
    test "a resource the pool no longer has is reported, not guessed at", context do
      ...
      {:ok, ledger} = Timetable.allocate(Timetable.ledger(), only)

      assert {:error, reason} = Ledger.arrangements(ledger, [context.studio])
      assert Timetable.explain(reason) =~ "Hall is allocated but is not in the pool"
    end
    ...
    test ":only narrows which sessions come back", context do
      ...
      ledger =
        Enum.reduce(arrangements, Timetable.ledger(), fn arrangement, ledger ->
