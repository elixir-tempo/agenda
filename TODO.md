...
* **Agenda cannot say how much a participant matters.** iCalendar's `ROLE` distinguishes required from optional attendance, and `Agenda.needs/2` has no way to express "Alice must be there, Bob would be nice". This is the most substantial missing concept found in the review, and it is one users will ask for by name.
